...
namespace noctalia::config {
  ...
  const std::vector<ConfigMigration>& configMigrations() {
    static const std::vector<ConfigMigration> migrations = {
        ...
        {
            // No-op: the lockscreen login box (and its show_password_hint setting) was
            // removed entirely. The slot must stay so the registry remains contiguous and
            // stored config versions (already migrated on user machines) keep matching.
            ...
            .apply = [](toml::table&, schema::Diagnostics&) {},
        },
        {
