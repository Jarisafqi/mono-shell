...
namespace {
  ...
  constexpr const char* kLogLevelEnv = "MONO_SHELL_LOG_LEVEL";
...
void initLogFile() {
  ...
  if (cacheHome != nullptr && cacheHome[0] != '\0') {
    dir = std::string(cacheHome) + "/mono-shell";
  } else if (home != nullptr && home[0] != '\0') {
    dir = std::string(home) + "/.cache/mono-shell";
  } else {
  ...
  const std::string logPath = dir + "/mono-shell.log";
  const std::string backupPath = dir + "/mono-shell.log.1";
