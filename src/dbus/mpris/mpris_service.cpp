...
bool MprisService::onPlayPausePlayer(const std::string& busName) {
  if (busName.empty()) {
    throw sdbus::Error(
        sdbus::Error::Name{"dev.monoshell.Mpris.Error.InvalidArgs"}, "player_bus_name must not be empty"
    );
  }
...
bool MprisService::onPreviousPlayer(const std::string& busName) {
  ...
  if (!ok) {
    throw sdbus::Error(
        sdbus::Error::Name{"dev.monoshell.Mpris.Error.NotSupported"}, "player does not support Previous"
    );
  }
  return true;
}
