...
namespace {
  ...
  constexpr auto kAgentObjectPath = "/org/monoshell/PolkitAuthenticationAgent";
...
using MonoshellPolkitListener = struct _MonoshellPolkitListener {
...
using MonoshellPolkitListenerClass = struct _MonoshellPolkitListenerClass {
  PolkitAgentListenerClass parent_class;
};

static void monoshell_polkit_listener_initiate_authentication(
...
static gboolean monoshell_polkit_listener_initiate_authentication_finish(
    PolkitAgentListener* listener, GAsyncResult* result, GError** error
);
static void monoshell_polkit_request_cancelled(GCancellable* cancellable, gpointer user_data) noexcept;

G_DEFINE_TYPE(MonoshellPolkitListener, monoshell_polkit_listener, POLKIT_AGENT_TYPE_LISTENER)

static void monoshell_polkit_listener_init(MonoshellPolkitListener* self) {
  self->owner = nullptr;
...
static void monoshell_polkit_listener_class_init(MonoshellPolkitListenerClass* klass) {
  auto* listenerClass = POLKIT_AGENT_LISTENER_CLASS(klass);
  listenerClass->initiate_authentication = monoshell_polkit_listener_initiate_authentication;
  listenerClass->initiate_authentication_finish = monoshell_polkit_listener_initiate_authentication_finish;
}
...
static void monoshell_polkit_listener_initiate_authentication(
  ...
  guardPolkitCallback("initiate_authentication", [&]() {
    auto* self = reinterpret_cast<MonoshellPolkitListener*>(listener);
    ...
    if (cancellable != nullptr) {
      request->cancelHandlerId =
          g_cancellable_connect(cancellable, G_CALLBACK(monoshell_polkit_request_cancelled), request.get(), nullptr);
    }
...
static void monoshell_polkit_request_cancelled(GCancellable* /*cancellable*/, gpointer user_data) noexcept {
  guardPolkitCallback("request_cancelled", [&]() {
    ...
    auto* listener = source != nullptr ? reinterpret_cast<MonoshellPolkitListener*>(source) : nullptr;
...
struct PolkitAgent::Impl {
  ...
  MonoshellPolkitListener* listener = nullptr;
  ...
  Impl() : context(g_main_context_default()) {
    listener = static_cast<MonoshellPolkitListener*>(g_object_new(monoshell_polkit_listener_get_type(), nullptr));
    listener->owner = this;
