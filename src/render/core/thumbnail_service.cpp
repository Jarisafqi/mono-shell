...
namespace {
  ...
  std::filesystem::path thumbnailCacheDir() {
    if (const char* xdg = std::getenv("XDG_CACHE_HOME"); xdg != nullptr && xdg[0] != '\0') {
      return std::filesystem::path(xdg) / "mono-shell" / "thumbnails";
    }
    if (const char* home = std::getenv("HOME"); home != nullptr && home[0] != '\0') {
      return std::filesystem::path(home) / ".cache" / "mono-shell" / "thumbnails";
    }
    return std::filesystem::path("/tmp") / "mono-shell" / "thumbnails";
  }
