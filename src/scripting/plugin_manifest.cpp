...
namespace scripting {
  ...
  namespace {
    ...
    constexpr std::array<std::pair<PluginEntryKind, std::string_view>, 5> kEntryKinds{{
    ...
    // settings editor: bar widgets edit per-instance, panels edit from the
    // plugin page. Launcher providers, shortcuts, and services are singletons
    // with no settings surface; use a plugin-level [[setting]] instead.
        ...
        if (const auto* settings = (*entryTable)["setting"].as_array()) {
          if (!entryKindSupportsSettings(kind)) {
            error = "entry '"
                ...
                + "' declares [["
                + std::string(tableName)
                + ".setting]], but entry-level settings are only supported for widget and "
                  "panel entries; move it to a plugin-level [[setting]]";
            return false;
          }
