...
namespace {
  ...
  void layoutBarSections(
    ...
    const bool flushToEdgeEnd =
        isVertical ? (instance.barConfig.position == "right") : (instance.barConfig.position == "bottom");
    ...
    auto finalizeCapsules = [isVertical, capsuleCross, flushToEdgeEnd,
      ...
      for (auto& run : runs) {
        ...
        const float contentX = flushToEdgeEnd && flushRunToEdge && isVertical ? (shellW - iw) : (shellW - iw) * 0.5f;
        const float contentY = flushToEdgeEnd && flushRunToEdge && !isVertical ? (shellH - ih) : (shellH - ih) * 0.5f;
    ...
    // The start section can opt out of the bar's main-axis padding so its widget (workspace
    // pills) hugs the bar's start edge while the end section keeps its own inset.
    const bool flushStartToBarEdge =
        !instance.startWidgets.empty() && instance.startWidgets.front()->barContentFlushToBarEdge();
    const float contentMainStart = (screenEdgeClick || flushStartToBarEdge) ? 0.0f : padding;
    ...
    if (screenEdgeClick) {
      if (isVertical) {
        instance.startSection->setPadding(flushStartToBarEdge ? 0.0f : paddingInsideSection, 0.0f, 0.0f, 0.0f);
        instance.endSection->setPadding(0.0f, 0.0f, paddingInsideSection, 0.0f);
      } else {
        instance.startSection->setPadding(0.0f, 0.0f, 0.0f, flushStartToBarEdge ? 0.0f : paddingInsideSection);
  ...
  if (targetInstance != nullptr && targetInstance->attachedPopupCount > 0) {
    ...
    case PointerEvent::Type::Axis:
      if (event.type == PointerEvent::Type::Button && event.button == BTN_RIGHT && event.pressed) {
        ...
        handleBarDeadZoneButton(*targetInstance, sx, sy, event.button, m_platform, m_actionDispatcher);
        return true;
      }
      break;
    }
