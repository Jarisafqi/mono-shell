...
public:
  ...
  // Whether the section containing this widget should be flush to the bar's start edge
  // (ignores the bar's main-axis padding on the start side). Used so workspace pills can
  // hug the bar edge while clock/battery keep their own padding.
  [[nodiscard]] virtual bool barContentFlushToBarEdge() const noexcept { return false; }
