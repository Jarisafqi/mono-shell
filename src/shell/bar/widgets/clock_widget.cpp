...
namespace {
  ...
  constexpr std::array<std::string_view, 12> kMonthAbbrevs{"JAN", "FEB", "MAR", "APR", "MAY", "JUN",
...
std::string ClockWidget::formatDateText() const {
  ...
  return std::format(
      "{}-{}-{}", local.tm_mday, kMonthAbbrevs[static_cast<std::size_t>(monthIndex)], local.tm_year + 1900
  );
}
