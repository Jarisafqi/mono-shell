...
namespace {
  ...
  [[nodiscard]] constexpr float workspaceLabelFontSize(bool /*minimal*/) {
    // Match the body font size used by the other bar widgets (clock, battery).
    return Style::fontSizeBody;
  }
  ...
  // Width of a 2-digit number ("10") measured at the given font. Every workspace
  // slot shares this reference width (plus padding): one uniform box size, with
  // all digits kept at their full font size and nothing shrunk.
  [[nodiscard]] float
  twoDigitWidth(Renderer& renderer, float fontSize, FontWeight weight, std::string_view fontFamily) {
    const TextMetrics tm = renderer.measureText("10", fontSize, weight, 0.0f, 0, TextAlign::Start, fontFamily);
    return std::max(tm.right - tm.left, tm.inkRight - tm.inkLeft);
  }

  // Shrinks the label font so its text fits inside the shared slot content
  // width; keeps a readable floor so wide labels never collapse to nothing.
  [[nodiscard]] constexpr float fitFontFor(float textWidth, float refWidth, float baseFontSize) noexcept {
    if (textWidth <= refWidth || textWidth <= 0.0f) {
      return baseFontSize;
    }
    return std::max(baseFontSize * (refWidth / textWidth), baseFontSize * 0.45f);
  }
...
WorkspacesWidget::WorkspacesWidget(
    ...
    : m_platform(platform), m_configService(config), m_output(output), m_displayMode(options.displayMode),
      ...
      m_hideWhenEmpty(options.hideWhenEmpty), m_pillScale(options.pillScale), m_labelScale(options.labelScale),
...
void WorkspacesWidget::applyItemVisualStyle(Item& item) {
  if (item.indicator != nullptr) {
    // Inactive pills are invisible; the active workspace gets a white pill.
    item.indicator->setVisible(true);
    item.indicator->setBorder(clearColorSpec(), 0.0f);
    item.indicator->setFill(item.visualWorkspace.active ? fixedColorSpec(rgbHex(0xFFFFFF)) : clearColorSpec());
  }
...
void WorkspacesWidget::rebuild(Renderer& renderer) {
  ...
  const float labelFontSize = workspaceLabelFontSize(m_minimal) * m_contentScale * m_labelScale;
  ...
  const float sidePadding = m_minimal ? padding * 2.0f : padding;
  // Every slot shares one box width: the widest single digit plus side padding.
  // Multi-digit labels are displayed at a reduced font so they fit that box.
  const float stdContentWidth = twoDigitWidth(
      renderer, labelFontSize, workspaceFontWeight(configuredFontWeight, m_minimal, false), labelFontFamily()
  );
  const float stdSlotWidth = std::round(stdContentWidth + sidePadding);
  ...
  for (std::size_t i = 0; i < entries.size(); ++i) {
    ...
    if (entry.showLabel) {
      ...
      slot.fontSize = fitFontFor(slot.textWidth, stdContentWidth, labelFontSize);
    }
  }
  ...
  for (std::size_t i = 0; i < entries.size(); ++i) {
    ...
    if (m_minimal) {
      ...
      if (!entry.showLabel) {
        ...
        // Uniform slot width for every workspace; labels too wide for it are
        // shown at the reduced font computed above.
        slot.inactiveWidth = stdSlotWidth;
        slot.activeWidth = stdSlotWidth;
      }
      if (entry.showLabel) {
        const FontWeight slotFontWeight = workspaceFontWeight(configuredFontWeight, m_minimal, entry.workspace.active);
        const TextMetrics tm = renderer.measureText(
    ...
    if (!entry.showLabel) {
      ...
      // Uniform slot width for every workspace; the label font was fitted above.
      slot.inactiveWidth = stdSlotWidth;
      slot.activeWidth = stdSlotWidth;
    }
  }
    ...
    if (!m_minimal) {
      ...
      item.indicator = static_cast<Box*>(area->addChild(
          ui::box({
              ...
              .configure = [ws](Box& box) {
                // Inactive pills stay invisible; the active workspace gets a white pill.
                box.setVisible(true);
                box.setFill(ws.active ? fixedColorSpec(rgbHex(0xFFFFFF)) : clearColorSpec());
...
void WorkspacesWidget::recalculateItemMetrics(
  ...
  float textWidth = 0.0f;
  float textHeight = 0.0f;
  float fittedFontSize = labelFontSize;
  if (item.showLabel) {
    const FontWeight slotFontWeight = workspaceFontWeight(configuredFontWeight, m_minimal, workspace.active);
    const TextMetrics tm =
        renderer.measureText(label, labelFontSize, slotFontWeight, 0.0f, 0, TextAlign::Start, labelFontFamily());
    textWidth = std::max(tm.right - tm.left, tm.inkRight - tm.inkLeft);
    textHeight = tm.bottom - tm.top;
  }
  // Uniform slot width (widest single digit + side padding); multi-digit labels
  // use a fitted font size so every workspace box matches.
  const float sidePadding = m_minimal ? padding * 2.0f : padding;
  const float stdContentWidth = twoDigitWidth(
      renderer, labelFontSize, workspaceFontWeight(configuredFontWeight, m_minimal, false), labelFontFamily()
  );
  const float stdSlotWidth = std::round(stdContentWidth + sidePadding);
  if (item.showLabel) {
    fittedFontSize = fitFontFor(textWidth, stdContentWidth, labelFontSize);
  }
  if (m_minimal) {
    const float minWidth = baseSize;
    if (!item.showLabel) {
      ...
      item.inactiveWidth = stdSlotWidth;
      item.activeWidth = stdSlotWidth;
    }
  ...
  if (item.text != nullptr) {
    ...
    if (item.showLabel) {
      ...
      item.text->setFontSize(fittedFontSize);
...
void WorkspacesWidget::applyItemLayout(Item& it) {
  ...
  // Center the pill content against the pill itself (no vertical nudge).
  constexpr float kTextAnchorNudge = 0.0f;
  ...
  if (m_isVertical) {
    ...
    float cursorY = (itemH - contentHeight) * 0.5f + kTextAnchorNudge;
  ...
  if (showText) {
    const float textY = contentY + (contentHeight - it.text->height()) * 0.5f + kTextAnchorNudge;
    it.text->setPosition(std::max(0.0f, cursorX), textY);
    cursorX += textWidth + iconGap;
  }
  if (showIcon) {
    it.icon->setSize(iconSize, iconSize);
    it.icon->setPosition(std::max(0.0f, cursorX), contentY + (contentHeight - iconSize) * 0.5f + kTextAnchorNudge);
  }
}
...
// noctadark outline style: the active workspace (and an urgent one) draw a
// solid outline; occupied/inactive workspaces draw no outline at all.
bool WorkspacesWidget::workspaceBorderVisible(const Workspace& workspace) const {
  return workspace.active || workspace.urgent;
}
...
ColorSpec WorkspacesWidget::workspaceTextColor(const Workspace& workspace) const {
  ...
  if (!m_minimal) {
    if (workspace.active) {
      // Active workspace sits on a white pill, so its label is black for contrast.
      return fixedColorSpec(rgbHex(0x000000));
    }
    // Unfilled (outline) pills: draw the text in the workspace color itself so
    // it stays visible on the bar surface instead of the on-fill readable color.
    return workspaceFillColor(workspace);
  }
  if (workspace.active) {
    if (m_activeUsesFocusedColor) {
      return m_focusedColor;
