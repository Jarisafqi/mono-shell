...
public:
  ...
  // Workspace pills hug the bar's start edge (skip the bar's main-axis padding), so they
  // sit flush against the bar edge. Clock/battery keep the bar padding.
  [[nodiscard]] bool barContentFlushToBarEdge() const noexcept override { return true; }
