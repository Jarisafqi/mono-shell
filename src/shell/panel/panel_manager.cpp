...
void PanelManager::registerIpc(IpcService& ipc) {
  ...
  ipc.registerHandler(
      ...
      "<id> [context]", "Toggle a panel by id, optionally with context (e.g. launcher /emo, launcher clipboard)"
  );
  ...
  ipc.registerHandler(
      ...
      "<id> [context]", "Open a panel by id, optionally with context (e.g. launcher /emo, launcher clipboard)"
  );
