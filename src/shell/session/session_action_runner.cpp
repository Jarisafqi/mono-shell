...
SessionActionRunner::SessionActionRunner(CompositorPlatform& platform, SessionActionHooks hooks)
    : m_platform(platform), m_hooks(std::move(hooks)) {}
...
void SessionActionRunner::invoke(const SessionPanelActionConfig& cfg) const {
  ...
  if (cfg.action == "lock_and_suspend") {
    if (!lockThenSuspendDetached()) {
      notify::error("Mono Shell", i18n::tr("session.errors.lock-title"), i18n::tr("session.errors.lock-body"));
    }
    return;
  }
  ...
  if (cfg.action == "lock") {
    kLog.info("lock: shell lock screen removed; use an external locker (e.g. hyprlock)");
    return;
  }
}
...
bool SessionActionRunner::lockThenSuspendDetached() const {
  kLog.info("lock-and-suspend: no shell lock screen; suspending directly");
  return requestSuspendDetached();
}
