...
namespace settings {
  ...
  bool widgetTypeRequiresNamedConfig(std::string_view type) {
    return type == "spacer" || resolvePluginWidget(type).has_value();
  }
  ...
  std::vector<WidgetPickerEntry> widgetPickerEntries(const Config& cfg) {
    ...
    for (const auto& [name, widget] : cfg.widgets) {
      ...
      // Only surface named instances of built-in multi-instance types (spacer).
  ...
  std::vector<WidgetSettingSpec> commonWidgetSettingSpecs(std::string_view shellFontFamily, bool populateFontCatalogs) {
    ...
    auto tooltip = withGroup(boolSpec("tooltip", true), "presentation");
    ...
    return {
        ...
        std::move(interactive),     std::move(tooltip),
        std::move(scale),           std::move(widgetColor),
  ...
  namespace {
    ...
    std::vector<WidgetSettingSpec> typeWidgetSettingSpecs(
      ...
      if (const auto* projection = findTypedWidgetDefinitionProjection(type)) {
        ...
        {
          auto labelSize = withGroup(doubleSpec("label_size", 1.0, 0.5, 2.0, 0.05), "workspaces.pills");
          labelSize.descriptionKey = "settings.widgets.settings.label-size.workspaces-description";
          labelSize.visibleWhen = pillStyleOnly;
