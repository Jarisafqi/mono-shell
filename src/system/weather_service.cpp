...
std::filesystem::path WeatherService::transportCacheDir() { return std::filesystem::path("/tmp") / "mono-shell-weather"; }

std::filesystem::path WeatherService::stateCacheFilePath() {
  if (const char* xdg = std::getenv("XDG_CACHE_HOME"); xdg != nullptr && xdg[0] != '\0') {
    return std::filesystem::path(xdg) / "mono-shell" / "weather.json";
  }
  if (const char* home = std::getenv("HOME"); home != nullptr && home[0] != '\0') {
    return std::filesystem::path(home) / ".cache" / "mono-shell" / "weather.json";
  }
  return std::filesystem::path("/tmp") / "mono-shell-weather-cache.json";
}
