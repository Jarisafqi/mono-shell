...
bool ToplevelSurface::initialize(wl_output* output, ToplevelSurfaceConfig config) {
  ...
  xdg_toplevel_set_title(m_toplevel, config.title.empty() ? "Mono Shell" : config.title.c_str());
  xdg_toplevel_set_app_id(m_toplevel, config.appId != nullptr ? config.appId : "dev.monoshell.MonoShell");
