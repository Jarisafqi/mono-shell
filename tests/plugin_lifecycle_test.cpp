...
int main() {
...

  {
    ...
    runtime.start(
        "=enable",
        "monoshell.state.set('loaded', true)\n"
        "function onEnable()\n"
        "  monoshell.state.set('enabled', true)\n"
        "end\n",
        {}
    );
...

  {
    ...
    runtime.start(
        ...
        "  monoshell.state.set('exit_signal', signal)\n"
        "  monoshell.state.set('exit_reason', reason)\n"
        "end\n",
        {}
    );
...

  {
    ...
    runtime.reload("=after-reload", "monoshell.state.set('reloaded', true)\n", {});
  ...
  ::setenv("MONO_SHELL_CONFIG_HOME", (root / "config").c_str(), 1);
  ::setenv("MONO_SHELL_STATE_HOME", (root / "state").c_str(), 1);
  ::setenv("MONO_SHELL_DATA_HOME", (root / "data").c_str(), 1);
  ...
  const auto soundPluginDir = root / "sound-plugin";
  {
    ...
    runtime.start(
        "=sound-outcomes",
        "local successAccepted = monoshell.sound.load('click', 'sounds/click.wav', function(ok, err)\n"
        "  monoshell.state.set('success_ok', ok)\n"
        "  monoshell.state.set('success_error_nil', err == nil)\n"
        "  if ok then monoshell.sound.play('click') end\n"
        "end)\n"
        "monoshell.state.set('success_accepted', successAccepted)\n"
        "local failureAccepted = monoshell.sound.load('broken', 'sounds/broken.wav', function(ok, err)\n"
        "  monoshell.state.set('failure_ok', ok)\n"
        "  monoshell.state.set('failure_error', err)\n"
        "  if ok then monoshell.sound.play('broken') end\n"
        "end)\n"
        "monoshell.state.set('failure_accepted', failureAccepted)\n",
        {}
    );
    ok =
        expect(
  ...
  unloadedSoundOwners.clear();
  {
    ...
    runtime.start(
        ...
        "local accepted = 0\n"
        "for index = 1, 8 do\n"
        "  if monoshell.sound.load('pending' .. index, 'sound.wav', function() end) then\n"
        "    accepted = accepted + 1\n"
        "  end\n"
        "end\n"
        "local duplicate = monoshell.sound.load('pending1', 'other.wav', function() end)\n"
        "local ninth = monoshell.sound.load('pending9', 'sound.wav', function() end)\n"
        "monoshell.state.set('accepted_count', accepted)\n"
        "monoshell.state.set('duplicate_accepted', duplicate)\n"
        "monoshell.state.set('ninth_accepted', ninth)\n",
        {}
    );
  ...
  unloadedSoundOwners.clear();
  {
    ...
    runtime.start(
        "=sound-unhealthy",
        "for index = 1, 5 do\n"
        "  monoshell.sound.load('failure' .. index, 'sound.wav', function()\n"
        "    error('deliberate callback failure')\n"
        "  end)\n"
        "end\n"
        "local accepted = monoshell.sound.load('after-errors', 'sound.wav', function(ok, err)\n"
        "  monoshell.state.set('completion_after_unhealthy', ok and err == nil)\n"
        "end)\n"
        "monoshell.state.set('after_errors_accepted', accepted)\n",
        {}
    );
    ok = expect(
             drainUntil([&] {
  ...
  std::uint64_t secondOwner = 0;
  {
    ...
    const std::string source = "monoshell.sound.load('click', 'click.wav', function(ok)\n"
                               "  if ok then monoshell.sound.play('click') end\n"
                               "end)\n";
  ...
  std::uint64_t staleOwner = 0;
  {
    ...
    onSoundLoad = [&runtime] {
      runtime.reload(
          "=sound-replacement",
          "monoshell.state.set('replacement_loaded', true)\n"
          "monoshell.state.set('stale_callback_reached', false)\n",
          {}
      );
    };
    runtime.start(
        "=sound-stale",
        "monoshell.sound.load('stale', 'stale.wav', function()\n"
        "  monoshell.state.set('stale_callback_reached', true)\n"
        "end)\n",
        {}
    );
    ok = expect(
...

  {
    ...
    lifecycleRuntime->start(
        "=manager-disable",
        "monoshell.state.set('disable_loaded', true)\n"
        "function onExit(_signal, reason)\n"
        "  monoshell.state.set('disable_reason', reason)\n"
        "end\n",
        {}
    );
    ...
    lifecycleRuntime->start(
        "=manager-uninstall",
        "monoshell.state.set('uninstall_loaded', true)\n"
        "function onExit(_signal, reason)\n"
        "  monoshell.state.set('uninstall_reason', reason)\n"
        "end\n",
        {}
    );
    ...
    lifecycleRuntime->start(
        "=legacy-disable",
        "monoshell.state.set('legacy_loaded', true)\n"
        "function onExit(_signal, reason)\n"
        "  monoshell.state.set('exit_reason', reason)\n"
        "end\n",
        {}
  ...
  ::unsetenv("MONO_SHELL_CONFIG_HOME");
  ::unsetenv("MONO_SHELL_STATE_HOME");
  ::unsetenv("MONO_SHELL_DATA_HOME");
  std::filesystem::remove_all(root);
  return ok ? 0 : 1;
}
