...
namespace {
  ...
  std::filesystem::path makeTempDir() {
    std::string pattern = (std::filesystem::temp_directory_path() / "mono-shell-plugin-manifest-XXXXXX").string();
...
int main() {
  ...
  ok = expectEq(
           ...
           "settings are only supported for widget and panel entries; move it to a plugin-level "
           "[[setting]]",
